// ============================================================
//  PS-15 スイッチボックス（1ボタン）v9 — 上下同一ハーフ設計
//  対象: Seimitsu PS-15
//  単位: mm
// ============================================================
//
...
/* ─── スナップフィット爪 ─── */
SNAP_ARM_L   = 5.0;   // アーム長（Y方向）
SNAP_ARM_W   = 2.0;   // アーム幅（X方向・壁厚方向）
SNAP_ARM_T   = 1.8;   // アーム厚（Z方向）
SNAP_HOOK    = 0.5;   // フック返し量（X方向）
SNAP_CHAMFER = 0.6;   // フック先端テーパー高さ（45°）
...
// ============================================================
//  モジュール: スナップフィット爪（左壁・XY平面内撓み）
//    左壁（X=0）内面から +X 方向に張り出す:
//      アーム本体（X=WALL〜WALL+SNAP_ARM_L）が Y 方向に延伸
...
module snap_arm_left() {
    ...
    translate([arm_x, arm_y, arm_z])
        cube([SNAP_ARM_L, SNAP_ARM_W, SNAP_ARM_T]);
    // フック: アーム先端（X = WALL + SNAP_ARM_L）に +X 方向突出
    // 下側: 45°テーパー（組み付けガイド）、上側: 垂直面（係合面）
    translate([arm_x + SNAP_ARM_L, arm_y, arm_z]) {
        // フック矩形本体（係合部）
        cube([SNAP_HOOK, SNAP_ARM_W, SNAP_ARM_T - SNAP_CHAMFER]);
        // 先端テーパー（45°）: Z が上がるにつれ X 方向が減少
        translate([0, 0, SNAP_ARM_T - SNAP_CHAMFER])
            linear_extrude(height = SNAP_CHAMFER, scale = [0, 1])
                square([SNAP_HOOK, SNAP_ARM_W]);
    }
}
...
module snap_arm_right() {
    ...
    translate([arm_x, arm_y, arm_z])
        cube([SNAP_ARM_L, SNAP_ARM_W, SNAP_ARM_T]);
    // フック: アーム先端（X = OUTER_W - WALL - SNAP_ARM_L）に -X 方向突出
    translate([arm_x - SNAP_HOOK, arm_y, arm_z]) {
        // フック矩形本体（係合部）
        cube([SNAP_HOOK, SNAP_ARM_W, SNAP_ARM_T - SNAP_CHAMFER]);
        // 先端テーパー（45°）
        translate([SNAP_HOOK, 0, SNAP_ARM_T - SNAP_CHAMFER])
            linear_extrude(height = SNAP_CHAMFER, scale = [0, 1])
                translate([-SNAP_HOOK, 0, 0])
                    square([SNAP_HOOK, SNAP_ARM_W]);
    }
}

// ============================================================
//  モジュール: スナップフィット受け穴（左壁）
//    左壁内面（X=WALL）側に掘る溝。
...
module snap_socket_left() {
    sock_clear = 0.2;
    sock_w = SNAP_ARM_W + 2 * sock_clear;      // Y 方向幅（クリア付き）
    sock_t = SNAP_ARM_T + 2 * sock_clear;      // Z 方向厚み（クリア付き）
    sock_depth = SNAP_ARM_L + 1.0;             // X 方向深さ（アーム長 + 余裕）
    hook_depth = SNAP_HOOK + sock_clear;       // フック受け段差深さ（X方向）
...
module snap_socket_right() {
    sock_clear = 0.2;
    sock_w = SNAP_ARM_W + 2 * sock_clear;
    sock_t = SNAP_ARM_T + 2 * sock_clear;
    sock_depth = SNAP_ARM_L + 1.0;
    hook_depth = SNAP_HOOK + sock_clear;
...
echo("=== PS-15 Box v9 上下同一ハーフ設計（開口カップ）===");
...
echo(str("スナップフィット爪:    アーム L", SNAP_ARM_L, " × W", SNAP_ARM_W, " × T", SNAP_ARM_T, " mm / フック返し ", SNAP_HOOK, " mm（左右壁・XY撓み）"));
